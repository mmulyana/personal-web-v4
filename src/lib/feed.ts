...
// Rewrite image paths from relative paths to Astro's optimized paths
  ...
  for (const match of matches) {
    ...
    if (src.startsWith('assets/')) {
      ...
      if (imageModule && imageModule.default) {
        try {
          // Use imageModule.default to get the ImageMetadata
          ...
          // Fall back to a basic absolute path
...
// Add a cover image to the start of the post
...
async function processPostsForFeed() {
  ...
  // Create a markdown processor with the same config as the project
  const processor = await createMarkdownProcessor({
    remarkPlugins,
    rehypePlugins,
    syntaxHighlight: false, // Keep in sync with astro.config.ts
  })
  ...
  const processedPosts = await Promise.all(
    posts.map(async (post) => {
      ...
      try {
        ...
        // Clean up the HTML, stripping it back down to plain markdown output
